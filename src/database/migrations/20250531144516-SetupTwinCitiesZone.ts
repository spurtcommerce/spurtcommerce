...
export class SetupTwinCitiesZone20250531144516 implements MigrationInterface {
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (usa) {
            ...
            const existingZone = await zoneRepository.findOne({
                where: {
            ...
            if (!existingZone) {
                ...
                const newZone = zoneRepository.create({
                    countryId: usaCountryId,
                    name: 'Twin Cities Metro',
                    code: 'MN-TC', // Or a more appropriate code if available
                    isActive: 1,
                });
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        if (usa) {
            const usaCountryId = usa.countryId;
            const zoneToDelete = await zoneRepository.findOne({
                where: {
                    name: 'Twin Cities Metro',
                    countryId: usaCountryId,
                },
            ...
            if (zoneToDelete) {
                ...
                await zoneRepository.remove(zoneToDelete);
