...
import { JsonController, Res, Req, Get, QueryParam, Authorized, Param, Delete, Put, BodyParam, Post, Body } from 'routing-controllers';
...
import { env } from '../../../env';
import { VendorOrderArchive } from '../../core/models/VendorOrderArchive';
import { VendorOrderArchiveLog } from '../../core/models/VendorOrderArchiveLog';
import { VendorOrderArchiveLogService } from '../../core/services/VendorOrderArchiveLogService';
...
export class VendorOrderController {
    ...
    @Authorized('vendor')
    public async vendorRecentOrderList(@QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('status') status: string, @QueryParam('count') count: number | boolean, @Req() request: any, @Res() response: any): Promise<any> {
    ...
    public async orderDetail(@Param('vendorOrderId') vendorOrderId: number, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        orderData.productList = await this.orderProductService.find({
            ...
            const vendorOrder = val.map(async (value: any) => {
                ...
                temp.orderFullfillmentStatusName = orderFullfillmentStatus?.name ?? '';
                temp.orderFullfillmentStatusColorCode = orderFullfillmentStatus?.colorCode ?? '';
                if (orderStatus) {
                    temp.orderStatusName = orderStatus.name;
                    temp.statusColorCode = orderStatus.colorCode;
                }
                temp.tags = temp.tags ?? '';
                temp.trackingUrl = temp.trackingUrl ?? '';
                temp.trackingNo = temp.trackingNo ?? '';
                return temp;
            });
    ...
    @Get('/archive-order-detail/:vendorOrderArchiveId')
    @Authorized('vendor')
    public async orderArchiveDetail(@Param('vendorOrderArchiveId') vendorOrderArchiveId: number, @Req() request: any, @Res() response: any): Promise<any> {
        const orderData = await this.vendorOrderArchiveService.findOne({
    ...
    @Authorized('vendor')
    public async orderStatusUpdate(@Param('vendorOrderId') vendorOrderId: number, @BodyParam('subOrderStatusId') subOrderStatusId: number, @BodyParam('fullfillmentStatusId') fullfillmentStatusId: number, @Req() request: any, @Res() response: any): Promise<any> {
        const updateVendorStatus = await this.updateOrderStatus(vendorOrderId, subOrderStatusId, fullfillmentStatusId, request);
     ...
     * HTTP/1.1 500 Internal Server Error
     */
    @Put('/:vendorOrderId/order-product')
    @Authorized('vendor')
    public async orderProductUpdate(@Param('vendorOrderId') vendorOrderId: number, @Body({ validate: true }) payload: { tags: string }, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        orderProduct.tags = payload.tags ?? orderProduct.tags;
    ...
    @Put('/order')
    @Authorized('vendor')
    public async updateOrder(@Body({ validate: true }) payload: { backOrder: number, vendorOrderIds: number[] }, @Res() response: any, @Req() request: any): Promise<any> {

        const validVendorOrders: any[] = await this.vendorOrdersService.findAll({ where: { vendorOrderId: In(payload.vendorOrderIds), vendorId: request.user.vendorId } });

        const orderIds: number[] = validVendorOrders.map((vendorOrder) => vendorOrder.orderId);
        ...
        for (const id of orderIds) {
            ...
            const orderProduct = await this.orderProductService.findOne({ where: { orderId: id } });
            const sku = await this.skuService.findOne({ where: { skuName: orderProduct.skuName } });
        ...
        await this.orderService.bulkUpdateByIds(validBackOrderIds, { backOrders: payload.backOrder });
    ...
    @Authorized('vendor')
    public async bulkOrderStatusUpdate(@Body({ validate: true }) params: { vendorOrderIds: number[], subOrderStatusId: number, fullfillmentStatusId: number }, @Req() request: any, @Res() response: any): Promise<any> {
        const vendorOrderIds = params.vendorOrderIds;
        for (const vendorOrderId of vendorOrderIds) {
            const updateVendorStatus = await this.updateOrderStatus(vendorOrderId, params.subOrderStatusId, params.fullfillmentStatusId, request);
            if (updateVendorStatus.status === 0) {
                return response.status(400).send(updateVendorStatus);
            }
        }
    ...
    public async updateOrderStatus(vendorOrderId: number, subOrderStatusId: number, fullfillmentStatusId: number, request: any): Promise<any> {
        const val = await this.orderStatusService.findOne({ where: { orderStatusId: subOrderStatusId } });
        ...
        if (orderStatusUpdate !== undefined) {
            ...
            const orderFullfillmentStatus = await this.orderFullfillmentStatusService.findOne(fullfillmentStatusId);
    ...
    @Authorized('vendor')
    public async vendorOrderBasedStatusList(
        @QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('vendorOrderLimit') vendorOrderLimit: number, @QueryParam('vendorOrderOffset') vendorOrderOffset: number,
        @QueryParam('keyword') keyword: string, @QueryParam('status') status: string, @QueryParam('count') count: number | boolean, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        const promise = orderStatusList.map(async (value: any) => {
            ...
            const orderList: any = await this.vendorOrdersService.listByQueryBuilder(vendorOrderLimit, vendorOrderOffset, selectOrder, whereConditions, searchConditions, relations, groupBy, sort, false, true);
     ...
     * HTTP/1.1 500 Internal Server Error
     */
    @Get('/vendor-orders-based-status-details/:orderStatusId')
    @Authorized('vendor')
    public async vendorOrderStatusDetails(@Param('orderStatusId') orderStatusId: string, @QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('count') count: number | boolean, @Req() request: any, @Res() response: any): Promise<any> {
        const selectOrder = [
            'VendorOrders.vendorOrderId as vendorOrderId',
            'VendorOrders.subOrderId as subOrderId',
            'orderDetail.createdDate as createdDate',
        ...
        if (count) {
            const orderListCount: any = await this.vendorOrdersService.listByQueryBuilder(limit, offset, selectOrder, whereConditions, searchConditions, relations, groupBy, sort, true, true);
            return response.status(200).send({
                status: 1,
        ...
        const orderList: any = await this.vendorOrdersService.listByQueryBuilder(limit, offset, selectOrder, whereConditions, searchConditions, relations, groupBy, sort, false, true);
        if (orderList) {
            return response.status(200).send({
                status: 1,
     ...
     * HTTP/1.1 500 Internal Server Error
     */
    @Get('/vendor-orders-based-status-count')
    @Authorized('vendor')
    public async orderListCount(@QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('vendorLimit') vendorLimit: number, @QueryParam('vendorOffset') vendorOffset: number, @QueryParam('keyword') keyword: string, @QueryParam('status') status: string, @QueryParam('count') count: number | boolean, @Req() request: any, @Res() response: any): Promise<any> {
        const select = ['orderStatusId', 'name', 'colorCode', 'isActive'];
        const search = [
            {
                name: 'name',
        ...
        const promise = orderStatusList.map(async (value: any) => {
            ...
            const orderList: any = await this.vendorOrdersService.listByQueryBuilder(0, 0, selectOrder, whereConditions, searchConditions, relations, groupBy, sort, true, true);
    ...
    public async orderLogList(@QueryParam('vendorOrderId') vendorOrderId: number, @Res() response: any): Promise<any> {
        ...
        const orderStatuss = await this.orderStatusService.findAll({ select: ['orderStatusId', 'name'], where: { isActive: 1 }, order: { priority: 'ASC' } });
    ...
    public async orderExportPdf(@QueryParam('vendorOrderId') vendorOrderId: number, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        const vendor = await this.vendorService.findOne({ where: { vendorId: orderData.vendorId } });
        orderData.vendor = vendor;
        orderData.productList = await this.orderProductService.find({ where: { orderProductId: orderData.orderProductId }, select: ['orderProductId', 'name', 'quantity', 'total', 'productId', 'productPrice', 'basePrice', 'taxType', 'taxValue', 'discountAmount', 'discountedAmount', 'couponDiscountAmount'] }).then((val) => {
            const vendorOrder = val.map(async (value: any) => {
                const ProductImage = await this.productImageService.findOne({ where: { productId: value.productId, defaultImage: 1 } });
                const temp: any = value;
                if (ProductImage) {
    ...
    @Authorized('vendor')
    public async archiveOrderListCount(
        @QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('keyword') keyword: string,
        @QueryParam('startDate') startDate: string, @QueryParam('endDate') endDate: string, @QueryParam('dateAdded') dateAdded: string,
    ...
    public async orderListtt(
        ...
        if (count) {
            const orderCount: any = await this.vendorOrdersService.listByQueryBuilder(limit, offset, select, whereConditions, searchConditions, relations, groupBy, sort, true, true);
            return response.status(200).send({
                status: 1,
     ...
     * HTTP/1.1 500 Internal Server Error
     */
    @Post('/order-list-export')
    @Authorized('vendor')
    public async orderListExport(@BodyParam('vendorOrderId') vendorOrderId: [], @Req() request: any, @Res() response: any): Promise<any> {
        const select = [
            'VendorOrders.vendorOrderId as vendorOrderId',
            'VendorOrders.orderId as orderId',
            'VendorOrders.vendorId as vendorId',
        ...
        orderList.map((orders) => {
            rows.push([orders.vendorOrderId, orders.orderPrefixId, orders.createdDate, orders.customerFirstName, orders.shippingCity, (orders?.currencySymbolLeft ?? orders.currencySymbolRight) + ' ' + orders.total, orders.orderStatusName]);
        });
    ...
    @Authorized('vendor')
    public async vendorInvoiceList(
    ...
    public async orderInvoiceExportPdf(@QueryParam('orderId') orderId: number, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        orderData.productList = await this.vendorInvoiceItemService.findAll({ where: { vendorInvoiceId: orderData.vendorInvoiceId }, select: ['vendorInvoiceItemId', 'vendorInvoiceId', 'orderProductId'] }).then((val) => {
            const vendorOrder = val.map(async (value: any) => {
                const orderProduct = await this.orderProductService.findOne({ where: { orderProductId: value.orderProductId }, select: ['orderProductId', 'name', 'quantity', 'total', 'productId', 'productPrice', 'basePrice', 'taxType', 'taxValue', 'discountAmount', 'discountedAmount', 'couponDiscountAmount', 'skuName'] });
        ...
        if (env.imageserver === 's3') {
            ...
            image = await this.imageService.resizeImageBase64(settingDetails.invoiceLogoPath + settingDetails.invoiceLogo, '150', '30');
        }
        orderData.logo = image;
        const htmlData = await this.pdfService.readHtmlToString('vendor-invoice', orderData);
    ...
    public async orderInvoiceExportSendEmailPdf(@BodyParam('orderId') orderId: number, @Req() request: any, @Res() response: any): Promise<any> {
        ...
        if (env.imageserver === 's3') {
            ...
            image = await this.imageService.resizeImageBase64(settingDetails.invoiceLogoPath + settingDetails.invoiceLogo, '100', '100');
        }
        orderData.logo = image;
        const htmlData = await this.pdfService.readHtmlToString('vendor-invoice', orderData);
        ...
        await this.pdfService.htmlPdf(htmlData, path);
        const attachments = [{
            name: fileName,
            path,
        }];
        ...
        await MAILService.sendMail(mailContents, customer.email, emailContent.subject, false, true, attachments);
     ...
     * HTTP/1.1 500 Internal Server Error
     */
    @Get('/vendor-failed-order-list')
    @Authorized('vendor')
    public async vendorFailedOrderList(
        @QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('keyword') keyword: string, @QueryParam('startDate') startDate: string, @QueryParam('endDate') endDate: string, @QueryParam('customerName') customerName: string, @QueryParam('orderId') orderId: string, @QueryParam('amount') amount: string,
        @QueryParam('orderStatus') orderStatus: string, @QueryParam('dateAdded') dateAdded: string, @QueryParam('count') count: number | boolean, @Req() request: any, @Res() response: any): Promise<any> {
        const startDateMin = moment(startDate).subtract(5, 'hours').subtract(30, 'minutes').format('YYYY-MM-DD HH:mm:ss');
        const date = endDate + ' 23:59:59';
        const endDateMin = moment(date).subtract(5, 'hours').subtract(30, 'minutes').format('YYYY-MM-DD HH:mm:ss');
        const select = [
        ...
        const failedOrderList: any = await this.vendorInvoiceService.listByQueryBuilder(limit, offset, select, whereConditions, searchConditions, relations, groupBy, sort, false, true);
        const failedOrderResponse = failedOrderList.map(async (value: any) => {
            const temp: any = value;
            temp.productItems = await this.vendorInvoiceItemService.findAll({
                where: { vendorInvoiceId: value.vendorInvoiceId },
            }).then((val) => {
                const orderProduct = val.map(async (values: any) => {
                    const products = await this.orderProductService.findOne({ orderProductId: values.orderProductId });
        ...
        const vendorFailedOrderList = await Promise.all(failedOrderResponse);
        if (failedOrderList) {
            const successResponse: any = {
                status: 1,
    ...
    @Get('/bulk-back-order-excel-list')
    @Authorized('vendor')
    public async bulkExportBackorder(@QueryParam('limit') limit: any, @QueryParam('offset') offset: any, @QueryParam('keyword') keyword: any, @Req() request: any, @Res() response: any): Promise<any> {
        const excel = require('exceljs');
        const workbook = new excel.Workbook();
    ...
    @Get('/back-order-excel-list')
    @Authorized('vendor')
    public async backOrderExport(@QueryParam('orderProductId') orderProductId: string, @Req() request: any, @Res() response: any): Promise<any> {
        const excel = require('exceljs');
        const workbook = new excel.Workbook();
        const worksheet = workbook.addWorksheet('Back Order Export excel Sheet');
        const rows = [];
        ...
        const orderList: any = await this.orderService.listByQueryBuilder(0, 0, select, whereConditions, searchConditions, relations, groupBy, sort, false, true);
        if (orderList.length === 0) {
            const errorResponse: any = {
                status: 0,
                message: 'file is empty',
        ...
        for (const order of orderList) {
            ...
            const left = order.currencySymbolLeft;
            if (left === null && right === null) {
                rows.push([order.orderPrefixId, order.orderProductPrefixId, order.customerFirstName, order.email, order.name, order.productPrice, order.quantity, (order?.currencySymbolLeft ?? order?.currencySymbolRight) + ' ' + order.total]);
            } else {
                if (left !== undefined) {
                    rows.push([order.orderPrefixId, order.orderProductPrefixId, order.customerFirstName, order.email, order.name, order.productPrice, order.quantity, left + order.total]);
                } else {
    ...
    @Authorized('vendor')
    public async failedOrderExportExcel(@QueryParam('orderId') orderId: string, @Req() request: any, @Res() response: any): Promise<any> {
        const excel = require('exceljs');
        const workbook = new excel.Workbook();
        ...
        const failedOrderList: any = await this.vendorInvoiceService.listByQueryBuilder(0, 0, select, whereConditions, [], relations, groupBy, sort, false, true);
        // Excel sheet column define
        ...
        for (const order of failedOrderList) {
            ...
            const productItems = await this.vendorInvoiceItemService.findAll({
     ...
     * HTTP/1.1 500 Internal Server Error
     */
    @Get('/bulk-failed-order-excel-list')
    @Authorized('vendor')
    public async BulkfailedOrderExportExcel(@Req() request: any, @Res() response: any): Promise<any> {
        const excel = require('exceljs');
        const workbook = new excel.Workbook();
     ...
     * HTTP/1.1 500 Internal Server Error
     */
    @Get('/vendor-order-status-list-based-on-parent')
    @Authorized('vendor')
    public async orderStatusListBasedOnParent(@QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('parentId') parentId: number, @Res() response: any): Promise<any> {
        ...
        const fullfillmentStatusIds = (await this.orderStatusToFullfillmentService.findAll({ where: { orderStatusId: parentId ?? 0 } })).map((orderStatusToFullfillment) => orderStatusToFullfillment.orderFulfillmentStatusId);
        const WhereConditions = [];
        WhereConditions.push(
            {
                name: 'id',
                value: In(fullfillmentStatusIds),
            },
            {
                name: 'isActive',
                value: 1,
        ...
        const orderStatusList = await this.orderFullfillmentStatusService.list(limit, offset, [], [], WhereConditions, false);
        if (orderStatusList) {
            const successResponse: any = {
                status: 1,
                message: 'Successfully got order status list based on parent',
     ...
     * HTTP/1.1 500 internal server error
     */
    @Get('/purchased-customer-list')
    @Authorized('vendor')
    public async purchasedCustomers(
        @QueryParam('offset') offset: number,
        @QueryParam('limit') limit: number,
        ...
        const vendorOrders = await this.vendorOrdersService.listByQueryBuilder(limit, offset, select, whereConditions, searchConditions, relations, groupBy, sort, false, true);
    ...
    @Get('/export-customer')
    @Authorized('vendor')
    public async exportVendorCustomer(@QueryParam('customerId') customerId: string, @Res() response: any, @Req() request: any): Promise<any> {
        const excel = require('exceljs');
        const workbook = new excel.Workbook();
        const worksheet = workbook.addWorksheet('Bulk Archive Order Archive Excel');
        // Excel sheet column define
     ...
     * HTTP/1.1 500 internal server error
     */
    @Get('/customer-purchased-product')
    @Authorized('vendor')
    public async customerPurchasedProductList(
        @Req() request: any,
        @Res() response: any,
        ...
        const orderProductList: any = await this.vendorOrdersService.listByQueryBuilder(limit, offset, select, whereCondition, searchConditions, relations, [], sort, false, true);
        if (count) {
            return response.status(200).send({ status: 1, message: 'Successfully got order product count', count: orderProductList.length });
        }
        const orderProductStatusList = await Promise.all(orderProductList.map(async (orderProduct: { orderProductId: number; }) => {
            const orderProductView: any = orderProduct;
            const orderStatus = await this.orderProductLogService.find({ where: { orderProductId: orderProduct.orderProductId }, relations: ['orderStatus'] });
     ...
     * HTTP/1.1 500 internal server error
     */
    @Get('/product-viewed-customer')
    @Authorized('vendor')
    public async vendorProductViewedCustomer(@QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @QueryParam('customerId') customerId: number, @QueryParam('productName') productName: string, @QueryParam('sku') sku: string, @QueryParam('count') count: number | boolean, @Res() response: any, @Req() request: any): Promise<any> {
        const select = [
            'product.sku as sku',
        ...
        const customerViewLogList = await this.vendorProductService.listByQueryBuilder(limit, offset, select, whereConditions, searchConditions, relation, groupBy, sort, count, true);
        if (!count) {
            const resultList = await Promise.all(customerViewLogList.map(async (value) => {
                const temp: any = value;
     ...
     */
    @Authorized('vendor')
    @Get('/export-order-invoice-list')
    public async exportOrderIvoiceList(@QueryParam('vendorInvoiceId') vendorInvoiceId: string, @QueryParam('limit') limit: number, @QueryParam('offset') offset: number, @Req() request: any, @Res() response: any): Promise<any> {
        const vendorInvoiceIds: any = vendorInvoiceId && vendorInvoiceId !== '' ? vendorInvoiceId.split(',') : [];
        const excel = require('exceljs');
        const workbook = new excel.Workbook();
        ...
        const orderInvoices: any = await this.vendorInvoiceService.listByQueryBuilder(limit, offset, select, whereConditions, [], relations, [], [], false, true);
