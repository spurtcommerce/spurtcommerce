...
let CategoryService = class CategoryService {
    ...
    delete(id) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            yield this.categoryRepository.repository.delete(id);
            return;
        });
    }
    // categoryList
    ...
    listByQueryBuilder(limit_1, offset_1) {
        return tslib_1.__awaiter(this, arguments, void 0, function* (limit, offset, select = [], whereConditions = [], searchConditions = [], relations = [], groupBy = [], sort = [], count = false, rawQuery = false) {
            const query = yield (0, typeormLoader_1.getDataSource)().getRepository(CategoryModel_1.Category).createQueryBuilder('Category');
            // Select
            if (select && select.length > 0) {
                query.select(select);
            }
