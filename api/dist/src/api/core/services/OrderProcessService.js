...
class OrderProcessService {
    ...
    processOrder(orderId, transactionsParams) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const EmailTemplateRepository = (0, typeormLoader_1.getDataSource)().getRepository(EmailTemplate_1.EmailTemplate);
            const orderProductRepository = (0, typeormLoader_1.getDataSource)().getRepository(OrderProduct_1.OrderProduct);
            const productImageRepository = (0, typeormLoader_1.getDataSource)().getRepository(ProductImage_1.ProductImage);
            ...
            const currencyRepository = (0, typeormLoader_1.getDataSource)().getRepository(Currency_1.Currency);
            const userRepository = (0, typeormLoader_1.getDataSource)().getRepository(User_1.User);
            const paymentRepository = (0, typeormLoader_1.getDataSource)().getRepository(Payment_1.Payment);
            const paymentItemsRepository = (0, typeormLoader_1.getDataSource)().getRepository(PaymentItems_1.PaymentItems);
            const CustomerCartRepository = (0, typeormLoader_1.getDataSource)().getRepository(CustomerCart_1.CustomerCart);
            const vendorPaymentRepository = (0, typeormLoader_1.getDataSource)().getRepository(VendorPayment_1.VendorPayment);
            const VendorProductsRepository = (0, typeormLoader_1.getDataSource)().getRepository(VendorProducts_1.VendorProducts);
            const VendorRepository = (0, typeormLoader_1.getDataSource)().getRepository(Vendor_1.Vendor);
            const VendorGlobalSettingRepository = (0, typeormLoader_1.getDataSource)().getRepository(VendorGlobalSettings_1.VendorGlobalSetting);
            const VendorOrdersRepository = (0, typeormLoader_1.getDataSource)().getRepository(VendorOrders_1.VendorOrders);
            const VendorInvoiceRepository = (0, typeormLoader_1.getDataSource)().getRepository(VendorInvoice_1.VendorInvoice);
            const VendorInvoiceItemRepository = (0, typeormLoader_1.getDataSource)().getRepository(VendorInvoiceItem_1.VendorInvoiceItem);
            const CustomerRepository = (0, typeormLoader_1.getDataSource)().getRepository(Customer_1.Customer);
            // ----
            const orderRepository = (0, typeormLoader_1.getDataSource)().getRepository(Order_1.Order);
            const orderData = yield orderRepository.findOne({ where: { orderId } });
            if (!orderData) {
                return {
                    status: 1,
            ...
            const currencySymbol = yield currencyRepository.findOne({ where: { currencyId: setting.storeCurrencyId } });
            ...
            for (i = 0; i < orderProduct.length; i++) {
                ...
                const productImageData = yield productRepository.findOne({ where: { productId: productInformation.productId } });
            ...
            const emailContent = yield EmailTemplateRepository.findOne({ where: { emailTemplateId: 5 } });
            const adminEmailContent = yield EmailTemplateRepository.findOne({ where: { emailTemplateId: 6 } });
            ...
            if (vendorInvoice.length > 0) {
                for (const vendInvoice of vendorInvoice) {
                    ...
                    for (const vendInvoiceItem of vendorInvoiceItem) {
                        ...
                        const vendorProductImageData = yield productRepository.findOne({ where: { productId: vendorProductInformation.productId } });
