...
let VendorInvoiceService = class VendorInvoiceService {
    ...
    findOne(findCondition) {
        ...
        return this.vendorInvoiceRepository.repository.findOne(findCondition);
    }
    // list
    list(limit, offset, select = [], search = [], whereConditions = [], count) {
        ...
        if (count) {
            return this.vendorInvoiceRepository.repository.count(condition);
        }
        return this.vendorInvoiceRepository.repository.find(condition);
    }
    // create
    create(vendorInvoice) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const newvendorInvoice = yield this.vendorInvoiceRepository.repository.save(vendorInvoice);
    ...
    update(id, vendorInvoice) {
        ...
        return this.vendorInvoiceRepository.repository.save(vendorInvoice);
    }
    // delete
    delete(id) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            this.log.info('Delete a vendorInvoice');
            const deleteVendorInvoice = yield this.vendorInvoiceRepository.repository.delete(id);
            return deleteVendorInvoice;
        });
    }
    // find Services
    findAll(data) {
        return this.vendorInvoiceRepository.repository.find(data);
    }
    listByQueryBuilder(limit_1, offset_1) {
        return tslib_1.__awaiter(this, arguments, void 0, function* (limit, offset, select = [], whereConditions = [], searchConditions = [], relations = [], groupBy = [], sort = [], count = false, rawQuery = false) {
            const query = yield (0, typeormLoader_1.getDataSource)().getRepository(VendorInvoice_1.VendorInvoice).createQueryBuilder();
            // Select
            if (select && select.length > 0) {
                query.select(select);
            }
...
exports.VendorInvoiceService = VendorInvoiceService = tslib_1.__decorate([
    (0, typedi_1.Service)(),
