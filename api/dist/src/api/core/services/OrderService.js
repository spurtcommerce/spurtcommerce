...
let OrderService = class OrderService {
    ...
    bulkUpdateByIds(ids, payload) {
        return this.orderRepository.repository.update({ orderId: (0, index_1.In)(ids) }, payload);
    }
    // order List
    ...
    delete(id) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            return yield this.orderRepository.repository.delete(id);
        });
    }
    // sales list
    salesList() {
    ...
    listByQueryBuilder(limit_1, offset_1) {
        return tslib_1.__awaiter(this, arguments, void 0, function* (limit, offset, select = [], whereConditions = [], searchConditions = [], relations = [], groupBy = [], sort = [], count = false, rawQuery = false) {
            const query = yield (0, typeormLoader_1.getDataSource)().getRepository(Order_1.Order).createQueryBuilder('Order');
            // Select
            if (select && select.length > 0) {
                query.select(select);
            }
