...
let CustomerController = class CustomerController {
    constructor(customerService, orderProductService, customerGroupService, settingService, productViewLogService, vendorService, vendorProductService, loginLogService, vendorOrdersService, emailTemplateService, exportLogService, imageService) {
            ...
            if (customerParam.password === customerParam.confirmPassword) {
                ...
                if (customerSave) {
                    if (+customerParam.mailStatus === 1) {
                        const emailContent = yield this.emailTemplateService.findOne({ where: { emailTemplateId: 4 } });
    ...
    customerList(limit, offset, name, keyword, status, email, customerGroup, date, count, response) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            const customerList = yield (0, customer_1.getCustomerList)((0, typeormLoader_1.getDataSource)(), select, limit, offset, name, status, email, customerGroup, keyword, date, count ? 1 : 0);
    ...
    downloadCustomerSample(request, response) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            // ------------------- Customer Template Sheet -------------------
            const templateSheet = workbook.addWorksheet('Customer_Sheet');
            templateSheet.columns = [
                ...
                { header: 'MailStatus', key: 'mailStatus', width: 20 },
            ];
            // Style header
            ...
            // ------------------- Instruction Sheet -------------------
            const instructionSheet = workbook.addWorksheet('Instruction Sheet');
            instructionSheet.columns = [
                { header: 'Bulk Upload Fields', key: 'field', width: 25 },
                { header: 'Instructions for Each Field', key: 'instruction', width: 100 },
            ];
            const instructionData = [
                { field: 'First Name', instruction: 'Enter the customer first name.' },
                { field: 'Last Name', instruction: 'Enter the customer last name.' },
                { field: 'Email', instruction: 'Enter a valid email address.' },
                { field: 'Mobile Number', instruction: 'Enter the customer mobile number.' },
                { field: 'Password', instruction: 'Enter the password.' },
                { field: 'Status (0/1)', instruction: '0 = Inactive, 1 = Active.' },
                { field: 'Mail Status (0/1)', instruction: '0 = Not Sent, 1 = Sent.' },
            ];
            instructionSheet.addRows(instructionData);
            // Style header row
            ['A1', 'B1'].forEach(cell => {
                const headerCell = instructionSheet.getCell(cell);
                headerCell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFFF00' } };
            ...
            // ------------------- Write & Download -------------------
            const fileName = path.join(process.cwd(), `Customer_Import_Sheet_${Date.now()}.xlsx`);
    ...
    uploadCustomerExcel(file, response) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            var _a;
            try {
                const random = Math.floor(Math.random() * 100 + 1);
                const fileName = `./customer_import_${random}.xlsx`;
                yield this.imageService.writeFile(fileName, new Uint8Array(file.buffer));
                const xlsxData = yield this.imageService.xlsxToJson(fileName);
                fs.unlinkSync(fileName);
                // Validation Loop
                for (const row of xlsxData) {
                    const rowNumber = xlsxData.indexOf(row) + 2; // Excel row number
                    if (!row.FirstName || !row.Email) {
                        return response.status(400).send({
                            status: 0,
                            row: rowNumber,
                            message: 'FirstName or Email missing',
                        });
                    }
                    const existingCustomer = yield this.customerService.findOne({
                        where: { email: row.Email.trim(), deleteFlag: 0 },
                    });
                    if (existingCustomer) {
                        return response.status(400).send({
                            status: 0,
                            row: rowNumber,
                            message: `Customer already exists: ${row.Email}`,
                        });
                    }
                ...
                for (const row of xlsxData) {
                    ...
                    newCustomer.firstName = row.FirstName.trim();
                    newCustomer.lastName = ((_a = row.LastName) === null || _a === void 0 ? void 0 : _a.trim()) || '';
                    newCustomer.email = row.Email.trim();
                    newCustomer.username = row.Email.trim();
                    newCustomer.mobileNumber = row.MobileNumber;
                    newCustomer.password = yield User_1.User.hashPassword(row.Password);
                    newCustomer.isActive = row.Status === 1 ? 1 : 0;
                    newCustomer.mailStatus = row.MailStatus === 1 ? 1 : 0;
                    newCustomer.siteId = 2;
                    newCustomer.deleteFlag = 0;
                    const savedCustomer = yield this.customerService.create(newCustomer);
                    if (savedCustomer && savedCustomer.mailStatus === 1) {
                        try {
                            const [emailContent, logo] = yield Promise.all([
                                this.emailTemplateService.findOne(4),
                                this.settingService.findOne({}),
                            ]);
                            const message = emailContent.content
                                .replace('{name}', savedCustomer.firstName)
                                .replace('{username}', savedCustomer.email)
                                .replace(/{storeName}/g, logo.siteName)
                                .replace('{password}', row.Password)
                                .replace(/{storeName}/g, logo.siteName);
                            const mailContents = {
                                logo,
                                emailContent: message,
                                redirectUrl: env_1.env.storeRedirectUrl,
                                productDetailData: '',
                            };
                            // Send email
                            mail_services_1.MAILService.sendMail(mailContents, savedCustomer.email, emailContent.subject, false, false, '');
                            console.log(`Welcome email sent to ${savedCustomer.email}`);
                        }
                        catch (emailError) {
                            console.error(`Error sending email to ${savedCustomer.email}:`, emailError);
                            // Continue with next customer even if email fails
                        }
                    }
                }
                return response.status(200).send({
                ...
                console.error('Error in uploadCustomerExcel:', error);
                return response.status(500).send({
                    status: 0,
                    message: 'Error processing customer import',
                    error: error.message,
                });
            }
        });
    ...
    excelCustomerView(customerId, status, request, response) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (!customerId || customerId === '') {
                ...
                customerAllIds = yield (0, customer_1.getCustomerList)((0, typeormLoader_1.getDataSource)(), select, 0, 0, '', status, '', '', '', '', 0).then((value) => tslib_1.__awaiter(this, void 0, void 0, function* () {
    ...
    vendorGraphList(vendorId, duration, response) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (vendorId !== 0) {
                ...
                vendor.productSoldCount = (_a = productSold[0].soldCount) !== null && _a !== void 0 ? _a : 0;
...
tslib_1.__decorate([
    (0, routing_controllers_1.Get)('/download-customer-sample'),
    (0, routing_controllers_1.Authorized)(),
    ...
    tslib_1.__metadata("design:returntype", Promise)
], CustomerController.prototype, "downloadCustomerSample", null);
tslib_1.__decorate([
    (0, routing_controllers_1.Post)('/upload-customer'),
    (0, routing_controllers_1.Authorized)(['admin', 'create-buyer']),
    ...
    tslib_1.__metadata("design:paramtypes", [Object, Object]),
    tslib_1.__metadata("design:returntype", Promise)
], CustomerController.prototype, "uploadCustomerExcel", null);
tslib_1.__decorate([
