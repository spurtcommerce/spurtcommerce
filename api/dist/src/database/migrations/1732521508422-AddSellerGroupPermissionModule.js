...
const PermissionModule_1 = require("../../api/core/models/PermissionModule");
class AddSellerGroupPermissionModule1732521508422 {
    up(queryRunner) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const repo = queryRunner.manager.getRepository(PermissionModuleGroup_1.PermissionModuleGroup);
            const PermissionModuleRepo = queryRunner.manager.getRepository(PermissionModule_1.PermissionModule);
            const exist = yield repo.findOne({ where: { slugName: 'market-place-vendor' } });
            if (exist) {
                exist.slugName = 'seller';
                exist.name = 'Seller';
                yield repo.save(exist);
            }
            const val2 = yield repo.findOne({ where: { slugName: 'seller-group' } });
            if (!val2) {
                ...
                const val3 = yield repo.save(sellerGroupPermissionModule);
                ...
                yield PermissionModuleRepo.save(SellerGroupPermissionSeed);
            }
        });
    }
    down(queryRunner) {
