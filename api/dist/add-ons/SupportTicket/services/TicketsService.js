...
let TicketsService = class TicketsService {
    constructor(ticketsRepository, log) {
    ...
    create(ticket) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            return this.ticketsRepository.repository.save(ticket);
        });
    }
    // find Condition
    find(ticket) {
        return this.ticketsRepository.repository.find(ticket);
    }
    // find one Condition
    findOne(ticket) {
        return this.ticketsRepository.repository.findOne(ticket);
    }
    // update ticket
    update(ticket) {
        return this.ticketsRepository.repository.save(ticket);
    ...
    list(limit, offset, select = [], relations = [], search = [], whereConditions = [], count) {
        ...
        if (count) {
            return this.ticketsRepository.repository.count(condition);
        }
        else {
            return this.ticketsRepository.repository.find(condition);
        }
    }
    // listByQueryBuilder
    listByQueryBuilder(limit_1, offset_1) {
        return tslib_1.__awaiter(this, arguments, void 0, function* (limit, offset, select = [], whereConditions = [], searchConditions = [], relations = [], groupBy = [], sort = [], count = false, rawQuery = false) {
            const query = yield (0, typeormLoader_1.getDataSource)().getRepository(Tickets_1.Tickets).createQueryBuilder();
            // Select
            if (select && select.length > 0) {
                query.select(select);
            }
    ...
    delete(id) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            return yield this.ticketsRepository.repository.delete(id);
        });
    }
};
...
exports.TicketsService = TicketsService = tslib_1.__decorate([
    ...
    tslib_1.__metadata("design:paramtypes", [TicketsRepository_1.TicketsRepository, Object])
], TicketsService);
