...
let PayPalController = class PayPalController {
    ...
    // Process PayPal Payment API
    /**
     * @api {get} /api/paypal-payment/process/:id Process PayPal Payment API
     * @apiGroup PayPal
     * @apiHeader {number} orderPrefixId
     * @apiSuccessExample {json} Success
     * HTTP/1.1 200 OK
     * {
     *      "status": "1"
     * }
     * @apiSampleRequest /api/paypal-payment/process/:id
     * @apiErrorExample {json} paypal error
    ...
    // proceed PayPal Payment API
    /**
     * @api {get} /api/paypal-payment/proceed/:id Proceed PayPal Payment API
     * @apiGroup PayPal
     * @apiHeader {number} orderId
     * @apiSuccessExample {json} Success
     * HTTP/1.1 200 OK
     * {
     *      "status": "1"
     * }
     * @apiSampleRequest /api/paypal-payment/proceed/:id
     * @apiErrorExample {json} paypal error
     * HTTP/1.1 500 Internal Server Error
     */
    proceed(orderId, response) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const orderRepository = (0, typeormLoader_1.getDataSource)().getRepository(Order_1.Order);
            const paypalOrderRepository = (0, typeormLoader_1.getDataSource)().getRepository(PaypalOrder_1.PaypalOrder);
            ...
            const orderDetail = yield orderRepository.findOne({ where: { orderId: +orderId } });
            if (!orderDetail) {
                return response.status(400).send({
                    status: 1,
                    message: 'Invalid Order Id',
    ...
    // Success PayPal Payment API
    /**
     * @api {get} /api/paypal-payment/success success PayPal Payment API
     * @apiGroup PayPal
     * @apiSuccessExample {json} Success
     * HTTP/1.1 200 OK
     * {
     *      "status": "1"
     * }
     * @apiSampleRequest /api/paypal-payment/success
     * @apiErrorExample {json} paypal error
     * HTTP/1.1 500 Internal Server Error
     */
    success(req, response) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            const paypalOrderRepository = (0, typeormLoader_1.getDataSource)().getRepository(PaypalOrder_1.PaypalOrder);
            const paypalOrderTransactionRepository = (0, typeormLoader_1.getDataSource)().getRepository(PaypalOrderTransaction_1.PaypalOrderTransaction);
            ...
            const orderRepository = (0, typeormLoader_1.getDataSource)().getRepository(Order_1.Order);
            const orderData = yield orderRepository.findOne({ where: { orderId: paypalDetail.orderId } });
            if (!orderData) {
                return response.status(400).send({
                    status: 1,
                    message: 'Invalid Order Id',
            ...
            // // ----
            // const ejs = require('ejs');
            // const html = ejs.render(fileContent, { storeUrl: env.storeRedirectUrl + orderData.orderPrefixId });
            return response.redirect(env_1.env.paymentSuccessRedirectUrl);
        });
    }
    // cancel PayPal Payment API
    /**
     * @api {get} /api/paypal-payment/cancel Cancel PayPal Payment API
     * @apiGroup PayPal
     * @apiSuccessExample {json} Success
     * HTTP/1.1 200 OK
     * {
     *      "status": "1"
     * }
     * @apiSampleRequest /api/paypal-payment/cancel
     * @apiErrorExample {json} paypal error
     * HTTP/1.1 500 Internal Server Error
     */
    cancel(req, response) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            // ----
            // const ejs = require('ejs');
            // const html = ejs.render(fileContent, { storeUrl: env.storeRedirectUrl });
            return response.redirect(env_1.env.paymentFailureRedirectUrl);
        });
    }
