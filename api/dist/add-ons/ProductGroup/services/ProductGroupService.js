...
const ProductGroupRepository_1 = require("../repositories/ProductGroupRepository");
const ProductGroup_1 = require("../models/ProductGroup");
const typeormLoader_1 = require("../../../src/loaders/typeormLoader");
let ProductGroupService = class ProductGroupService {
    constructor(productGroupRepository, log) {
    ...
    create(productGroup) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            this.log.info('Create a new productGroup ');
            return this.productGroupRepository.repository.save(productGroup);
        });
    }
    // find Condition
    find(productGroup) {
        return this.productGroupRepository.repository.find(productGroup);
    }
    // find one Condition
    findOne(productGroup) {
        return this.productGroupRepository.repository.findOne(productGroup);
    }
    // update productGroup
    update(id, productGroup) {
        return this.productGroupRepository.repository.update(id, productGroup);
    }
    // productGroup List
    list(limit, offset, select = [], relations = [], search = [], whereConditions = [], count) {
        ...
        if (count) {
            return this.productGroupRepository.repository.count(condition);
        }
        else {
            return this.productGroupRepository.repository.find(condition);
        }
    }
    // listByQueryBuilder
    listByQueryBuilder(limit_1, offset_1) {
        return tslib_1.__awaiter(this, arguments, void 0, function* (limit, offset, select = [], whereConditions = [], searchConditions = [], relations = [], groupBy = [], sort = [], count = false, rawQuery = false) {
            const query = yield (0, typeormLoader_1.getDataSource)().getRepository(ProductGroup_1.ProductGroup).createQueryBuilder('productGroup');
            // Select
            if (select && select.length > 0) {
                query.select(select);
            }
    ...
    delete(condition) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            return yield this.productGroupRepository.repository.delete(condition);
        });
    }
};
...
exports.ProductGroupService = ProductGroupService = tslib_1.__decorate([
    ...
    tslib_1.__metadata("design:paramtypes", [ProductGroupRepository_1.ProductGroupRepository, Object])
], ProductGroupService);
//# sourceMappingURL=ProductGroupService.js.map