...
let SupplierService = class SupplierService {
    constructor(supplierServiceRepository) {
        this.supplierServiceRepository = supplierServiceRepository;
    }
    // create
    create(data) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            return this.supplierServiceRepository.repository.save(data);
        });
    }
    save(data) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
    ...
    // findOne
    findOne(data) {
        return this.supplierServiceRepository.repository.findOne(data);
    }
    // update
    update(id, data) {
        data.id = id;
    ...
    list(limit = 0, offset = 0, select = [], relation = [], whereConditions = [], searchCondition, count) {
        ...
        condition.where = (qb) => {
            if (whereConditions && whereConditions.length > 0) {
                whereConditions.forEach((item) => {
                    ...
                    else if (item.op === 'like' && item.value !== undefined) {
        ...
        if (count) {
            return this.supplierServiceRepository.repository.count(condition);
        }
        else {
            return this.supplierServiceRepository.repository.find(condition);
        }
    }
    // delete
    delete(id) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const newSupplier = yield this.supplierServiceRepository.repository.update(id, { isActive: 0, isDelete: 1 });
            return newSupplier;
        });
    }
    ...
    listByQueryBuilder(limit_1, offset_1) {
        return tslib_1.__awaiter(this, arguments, void 0, function* (limit, offset, select = [], whereConditions = [], searchConditions = [], relations = [], groupBy = [], sort = [], count = false, rawQuery = false) {
            const query = yield (0, typeormLoader_1.getDataSource)().getRepository(Supplier_1.Supplier).createQueryBuilder();
            // Select
            if (select && select.length > 0) {
                query.select(select);
            }
    ...
    // find All
    find(data) {
        return this.supplierServiceRepository.repository.find(data);
    }
};
exports.SupplierService = SupplierService;
exports.SupplierService = SupplierService = tslib_1.__decorate([
    (0, typedi_1.Service)(),
    tslib_1.__metadata("design:paramtypes", [SupplierRepository_1.SupplierRepository])
], SupplierService);
