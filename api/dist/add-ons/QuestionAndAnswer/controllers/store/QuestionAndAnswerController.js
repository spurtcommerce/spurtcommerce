...
let QuestionAndAnswerController = class QuestionAndAnswerController {
    constructor(productQuestionService, productAnswerService, customerService, userService, productAnswerLikeService, settingsService, productService, vendorProductService, emailTemplateService, answerAbuseReasonService, answerReportAbuseService, vendorService, productImageService, currencyService, skuService) {
    ...
    createQuestion(questionParam, request, response) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            const productDetail = yield this.productService.findOne({
                where: { productId: questionParam.productId, isActive: 1 },
            });
            if (!productDetail) {
                const errorResponse = {
                    status: 1,
            ...
            const sku = yield this.skuService.findOne({ where: { id: questionParam.skuId } });
            if (!sku) {
                const errorResponse = {
                    status: 0,
            ...
            const emailContent = yield this.emailTemplateService.findOne({ where: { emailTemplateId: 17 } });
            const adminId = [];
            ...
            const message = emailContent.content.replace('{name}', 'Admin')
                .replace('{question}', questionParam.question)
            ...
            const vendorProduct = yield this.vendorProductService.findOne({ where: { productId: questionParam.productId, reuse: (0, typeorm_1.IsNull)() } });
            if (vendorProduct) {
                const vendor = yield this.vendorService.findOne({ where: { vendorId: vendorProduct.vendorId }, relations: ['customer'] });
                mailContents.redirectUrl = env_1.env.vendorRedirectUrl;
                mailContents.emailContent = message.replace('Admin', vendor.customer.firstName);
     ...
     * @apiErrorExample {json} updateLikeStatus error
     * HTTP/1.1 500 Internal Server Error
     */
    updateLikeStatus(answerId, type, request, response) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (type === 2) {
                ...
                if (answerLike) {
                    yield this.productAnswerLikeService.delete({ id: answerLike.id });
                }
            }
            else if (type === 1) {
                const findExist = yield this.productAnswerLikeService.findOne({
                ...
                if (answerDislike) {
                    yield this.productAnswerLikeService.delete({ id: answerDislike.id });
                }
            }
            let questionSave;
            if (type !== 0) {
                ...
                productAnswer.isActive = 1;
                productAnswer.customerId = request.user.id;
                questionSave = yield this.productAnswerLikeService.create(productAnswer);
            }
            ...
            if (type === 0) {
                const answerLikeRemove = yield this.productAnswerLikeService.findOne({
                    where: {
                        answerId, customerId: request.user.id,
                    },
                });
                if (answerLikeRemove) {
                    yield this.productAnswerLikeService.delete({ id: answerLikeRemove.id });
                }
                const likeExist = yield this.productAnswerLikeService.findLikeCount(answerId);
                const disLikeExist = yield this.productAnswerLikeService.findDislikeCount(answerId);
                answer.likes = likeExist.likeCount;
                answer.dislikes = disLikeExist.dislikeCount;
            }
            yield this.productAnswerService.create(answer);
            if (questionSave || type === 0) {
                const successResponse = {
                    status: 1,
     ...
     * @apiErrorExample {json} Create Answer AbuseReporterror
     * HTTP/1.1 500 Internal Server Error
     */
    createAnswerAbuseReport(abuseReport, request, response) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            report.customerId = request.user.id;
            report.isActive = 1;
            ...
            const customer = yield this.customerService.findOne({ where: { id: request.user.id } });
            const emailContent = yield this.emailTemplateService.findOne({ where: { emailTemplateId: 19 } });
            const logo = yield this.settingsService.findOne({ where: {} });
            const productDetail = yield this.productService.findOne({
    ...
    // Question List API
    /**
     * @api {Get} /api/store-question-answer/question-list/:skuId Question List API
     * @apiGroup Store Question Answer
     ...
     *    "status": "1"
     *  }
     * @apiSampleRequest /api/store-question-answer/question-list/:skuId
     * @apiErrorExample {json} questionList error
     * HTTP/1.1 500 Internal Server Error
     */
    questionList(limit, offset, keyword, skuId, count, request, response) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            const select = ['questionId', 'productId', 'question', 'referenceId', 'type', 'isActive', 'skuId'];
            ...
            const promise = questionList.map((result) => tslib_1.__awaiter(this, void 0, void 0, function* () {
                ...
                const temp = result;
                const answers = yield this.productAnswerService.findAll({
                    select: ['questionId', 'answerId', 'answer', 'referenceId', 'likes', 'dislikes', 'type', 'createdDate', 'isActive', 'defaultAnswer'],
                    where: { questionId: result.questionId, isActive: 1 },
                });
                if (answers.length > 0) {
                    temp.answerList = [];
                    answers.forEach((answer) => tslib_1.__awaiter(this, void 0, void 0, function* () {
                        if (request.id) {
                ...
                if (type && type === 3) {
                    ...
                    const customer = yield this.customerService.findOne({
                        select: ['id', 'firstName', 'avatar', 'avatarPath', 'city', 'createdDate'],
                    ...
                    const adminUser = yield this.userService.findOne({
                        select: ['userId', 'firstName', 'avatar', 'avatarPath', 'createdDate'],
     ...
     * @apiErrorExample {json} answerList error
     * HTTP/1.1 500 Internal Server Error
     */
    ...
    productDetail(productslug, request, response) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            productDetails.questionList = yield this.productQuestionService.findAll({
                select: ['questionId', 'productId', 'question', 'type', 'referenceId', 'createdDate', 'skuId'],
                where: { productId: productDetail.productId, isActive: 1 }, relations: ['productAnswer'],
                limit: 4,
            }).then((val) => {
...
tslib_1.__decorate([
    ...
    (0, routing_controllers_1.Get)('/question-list/:skuId'),
    tslib_1.__param(0, (0, routing_controllers_1.QueryParam)('limit')),
    tslib_1.__param(1, (0, routing_controllers_1.QueryParam)('offset')),
    tslib_1.__param(2, (0, routing_controllers_1.QueryParam)('keyword')),
    tslib_1.__param(3, (0, routing_controllers_1.Param)('skuId')),
...
exports.QuestionAndAnswerController = QuestionAndAnswerController = tslib_1.__decorate([
