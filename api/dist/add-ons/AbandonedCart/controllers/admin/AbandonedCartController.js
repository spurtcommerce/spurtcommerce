...
let AbandonedCartController = class AbandonedCartController {
     ...
     * @apiErrorExample {json} LiveCartList error
     * HTTP/1.1 500 Internal Server Error
     */
    liveCartList(response, keyword, limit, offset, startDate, endDate, count, type, emailId, customerName) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (customerName) {
                searchAbandoned.push({
                    name: [`Concat(cust.firstName, ' ', (CASE when cust.lastName is not null then cust.lastName else '' end))`],
                    value: customerName.trim(),
                });
            }
            if (emailId === null || emailId === void 0 ? void 0 : emailId.trim()) {
                searchAbandoned.push({
                    name: 'cust.email',
                    value: [emailId],
                });
     ...
     * @apiGroup Admin Cart
     * @apiHeader {String} Authorization
     * @apiParam (Request body) {String} ip ip
     * @apiParam (Request body) {String} type type
     ...
     * @apiErrorExample {json} CartDetail error
     * HTTP/1.1 500 Internal Server Error
     */
        ...
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (id === 0) {
                ...
                customerCart = yield this.customerCartService.find({
                    select: ['name', 'total', 'quantity', 'productPrice', 'productId', 'ip', 'createdDate', 'modifiedDate'],
                ...
                customerCartDetails = yield this.customerService.findOne({
                    select: ['firstName', 'lastName', 'email', 'ip', 'createdDate', 'modifiedDate'],
                    where: { id },
                });
                ...
                customerCart = yield this.customerCartService.find({
                    select: ['name', 'total', 'quantity', 'productPrice', 'productId', 'createdDate', 'modifiedDate'],
                    where: whereCondition,
                });
                if (!customerCart.length) {
    ...
    // Send-E-mail
    /**
     * @api {post} /api/admin-cart/abandoned-cart-email Send E-mail API
     * @apiGroup Admin Cart
     * @apiHeader {String} Authorization
     ...
     * @apiErrorExample {json} EmailToCustomer error
     * HTTP/1.1 500 Internal Server Error
     */
    emailToCustomer(customerId, message, response) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            const emailContent = yield this.emailTemplateService.findOne({ where: { emailTemplateId: 30 } });
            const storeMailContents = {};
            storeMailContents.logo = yield this.settingService.findOne({ where: {} });
            const currencySymbol = yield this.currencyService.findOne({ where: { currencyId: storeMailContents.logo.storeCurrencyId } });
     ...
     * @apiErrorExample {json} CartListExcelExporterror
     * HTTP/1.1 500 Internal Server Error
     */
    ...
    // Cart Update
    /**
     * @api {put} /api/admin-cart/:type To Update Cart
     * @apiGroup Admin Cart
     * @apiHeader {String} Authorization
     * @apiParam (Request body) {Number} customerId Customer ID
     ...
     * @apiErrorExample {json} DeleteCart error
     * HTTP/1.1 500 Internal Server Error
     */
...
tslib_1.__decorate([
    ...
    tslib_1.__param(8, (0, routing_controllers_1.QueryParam)('emailId')),
    tslib_1.__param(9, (0, routing_controllers_1.QueryParam)('customerName')),
    tslib_1.__metadata("design:type", Function),
    tslib_1.__metadata("design:paramtypes", [Object, String, Number, Number, String, String, Boolean, String, String, String]),
...
exports.AbandonedCartController = AbandonedCartController = tslib_1.__decorate([
