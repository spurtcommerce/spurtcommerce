...
const CreateCartRequest_1 = require("../store/requests/CreateCartRequest");
...
let CustomerAbandonedCartController = class CustomerAbandonedCartController {
    ...
    /**
     * @api {post} /api/guest-cart Add to Guest Cart API
     * @apiGroup Guest Cart
     ...
     * @apiErrorExample {json} AddGuestCart error
            ...
            if (findOption) {
                if (cartParam.type && cartParam.type === 'new') {
                    if (cartParam.quantity === 0) {
                        yield this.customerCartService.delete({ id: findOption.id });
                        const deleteCart = {
                            status: 1,
    ...
    /**
     * @api {put} /api/guest-cart Guess Cart API
     * @apiGroup Guest Cart
     * @apiHeader {String} Authorization
     * @apiParam (Request body) {String} customer customer
     * @apiParamExample {json} Input
     * {
     *      "id": ""
     * }
     ...
     * }
     * @apiSampleRequest /api/guest-cart
     * @apiErrorExample {json} GuestToCustomert error
     * HTTP/1.1 500 Internal Server Error
     */
...
exports.CustomerAbandonedCartController = CustomerAbandonedCartController = tslib_1.__decorate([
