...
@Component({
  selector: 'app-add-fullfillment-status',
  templateUrl: './add-fullfillment-status.component.html',
  styleUrl: './add-fullfillment-status.component.scss'
})
export class AddFullfillmentStatusComponent {
// deconrators 
@Input() id: any;
...
add():void {
  ...
  if (this.orderStatusInfo[0] && this.orderStatusInfo[0].id) {
    ...
    this.Orderfullfillmentsandbox.manageFullFillmentUpdate(params);
    this.Orderfullfillmentsandbox.manageFullFillmentUpdate$.subscribe(data => {
    ...
    this.Orderfullfillmentsandbox.manageFullFillmentAdd(params);
    this.Orderfullfillmentsandbox.manageFullFillmentAdd$.subscribe(data => {
      if (data && data.status === 1) {
        this.modalService.close('close');
        this.orderfullfillmentlist();
      }
...
private editOrderStatusList():void {
  this.orderStatusInfo.push(this.Orderfullfillmentservice.getManageFullFillment());
